...
It also installs **Harness Remote Access** in the Start menu. That entry opens a computer-only pairing page with a locally generated QR code and a copyable fallback link. Scan it from a phone connected to the same Wi-Fi, then add Harness to the phone's home screen. The phone receives the full native DSH surface—sessions, live output, composer, editing, models, permissions, goals, and usage—not a separate dashboard. The QR code is generated in your browser and the pairing token is never sent to a third-party QR service.
