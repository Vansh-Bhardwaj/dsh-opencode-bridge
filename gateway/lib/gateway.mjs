...
export async function defaultRouteAddresses() {
  if (process.platform !== 'win32') return privateAddresses().slice(0, 1);
  try {
    ...
    for (const line of stdout.split(/\r?\n/)) {
      ...
      if (match && isPrivateIpv4(match[1])) routes.push({ address: match[1], metric: Number(match[2]) });
    }
    routes.sort((a, b) => a.metric - b.metric);
    return routes.length ? [routes[0].address] : privateAddresses().slice(0, 1);
  } catch {
    return privateAddresses().slice(0, 1);
  }
}
...
export function accessPage(urls, consumed) {
  const rows = urls.slice(0, 1).map((url) => {
...
export function pairingUrls({ addresses, port, secure, secret, pairToken, ttlMs = 15 * 60 * 1000 }) {
  ...
  return addresses.filter(isPrivateIpv4).slice(0, 1).map((address) => `${secure ? 'https' : 'http'}://${address}:${port}/_bridge/pair?token=${encodeURIComponent(token)}`);
}
